...
public:
	...
	void buttonClicked(Button* b) override
	{
		...
		{
			// respond to button1
			commandManager.invokeDirectly(CommandIDs::showAudioSettings, true);
		}
		else if (b == &menuButton2)
		{
			// respond to button2
			commandManager.invokeDirectly(CommandIDs::open, true);

		}
		else if (b == &menuButton3)
		{
			for (int i = graph.getNumFilters(); --i >= 0;)
			{
				const AudioProcessorGraph::Node::Ptr f(graph.getNode(i));
				if (f->getProcessor()->getName() == "MiddlePlugin") {
					if (PluginWindow* const w = PluginWindow::getWindowFor(f, PluginWindow::Normal, graph.getGraph()))
						w->toFront(true);
				}
